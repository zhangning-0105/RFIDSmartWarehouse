...
{
    public partial class Outhouse : Form
    {
        public Outhouse()
        {
            InitializeComponent();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (tids.Count == Convert.ToInt32(strcout))
                {
                    ...
                    if ( DatabaseHelper.QueryProdcutsAndUpdateOuthouseOrderstaus(tids, productname, ordernum))
                    {
                    ...
                    {
                        MessageBox.Show("出库失败，读取的产品tid中有已出库的产品或者更新出库信息失败");
                    }

                }
                else
                {
                    MessageBox.Show("已读取产品数量与出库订单数量不符合，请重新读取后再出库");
                }
